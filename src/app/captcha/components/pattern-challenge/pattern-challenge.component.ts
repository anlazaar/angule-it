...
export class PatternChallengeComponent implements OnInit {
  ...
  generatePattern() {
    ...
    const commonShape = this.sanitizer.bypassSecurityTrustHtml(shuffledShapes[0]);
    const oddShape = this.sanitizer.bypassSecurityTrustHtml(shuffledShapes[1]);
    
    this.items = Array(6).fill(null).map(() => ({ icon: commonShape, isOdd: false }));
    const oddIndex = Math.floor(Math.random() * 6);
    this.items[oddIndex] = { icon: oddShape, isOdd: true };
  ...
  verify(index: number) {
    ...
    if (this.items[index].isOdd) {
      ...
      this.hasError = true;
      setTimeout(() => this.generatePattern(), 800);
    }
  }
}
